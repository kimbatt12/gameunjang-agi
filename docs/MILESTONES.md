...
- [x] LLM scope 수락 이후 compact API 후보 메타데이터를 전달해 LLM-assisted route 선택을 수행하고 deterministic fallback을 유지한다.
- [x] 지역명, 관광 카테고리, 동의어 기반 라우팅을 추가한다.
- [x] 지역이 감지되지 않은 관광 질문은 서울 기본값 대신 전국구로 라우팅한다.
...
- [x] `/api/chat` 관광 답변 경로에서 한국관광공사 API를 실제 호출한다. Vercel Preview에서 `강릉 2박 3일 코스` 요청이 200으로 응답하고 실제 TourAPI 항목을 반환함을 확인했다.
