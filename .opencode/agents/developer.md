...
- Frontend and backend stacks may differ. Do not assume a root Node package or shared npm workflow.
...
6. For repository-level config changes, run syntax checks for changed config files when available and `git diff --check`.
7. Before any push, ask `@code-reviewer` to review the diff, address findings, and rerun relevant validation.
